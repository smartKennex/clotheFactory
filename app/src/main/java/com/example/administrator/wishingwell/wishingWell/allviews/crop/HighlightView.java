...
package com.example.administrator.wishingwell.wishingWell.allviews.crop;
...
import com.example.administrator.wishingwell.wishingWell.utils.DimensionUtil;
...
class HighlightView {
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean maintainAspectRatio) {
        ...
        outlinePaint.setColor(viewContext.getResources().getColor(R.color.crop_outline_color));
        outlinePaint.setAntiAlias(true);
        outlineWidth = OUTLINE_DP;//dpToPx(OUTLINE_DP);

        handlePaint.setColor(viewContext.getResources().getColor(R.color.crop_handle_color));

        handlePaint.setStyle(Paint.Style.FILL);
        handlePaint.setAntiAlias(true);
        handlePaint.setStrokeWidth(dpToPx(OUTLINE_DP * 2));
...
//            }
    ...
    private void drawHandles(Canvas canvas) {
        final float handleLength = dpToPx(24.0f);
        final float offset = handlePaint.getStrokeWidth() / 2;

        canvas.drawLine(drawRect.left, drawRect.top + drawRect.height() / 3,
                drawRect.right, drawRect.top + drawRect.height() / 3, outlinePaint);

        canvas.drawLine(drawRect.left, drawRect.top + drawRect.height() * 2 / 3,
                drawRect.right, drawRect.top + drawRect.height() * 2 / 3, outlinePaint);


        canvas.drawLine(drawRect.left + drawRect.width() / 3, drawRect.top,
                drawRect.left + drawRect.width() / 3, drawRect.bottom, outlinePaint);

        canvas.drawLine(drawRect.left + drawRect.width() * 2 / 3, drawRect.top,
                drawRect.left + drawRect.width() * 2 / 3, drawRect.bottom, outlinePaint);

        canvas.drawLine(drawRect.left - offset, drawRect.top,
                drawRect.left + handleLength, drawRect.top, handlePaint);
        canvas.drawLine(drawRect.left, drawRect.top - offset,
                drawRect.left, drawRect.top + handleLength, handlePaint);

        canvas.drawLine(drawRect.right + offset, drawRect.top,
                drawRect.right - handleLength, drawRect.top, handlePaint);
        canvas.drawLine(drawRect.right, drawRect.top - offset,
                drawRect.right, drawRect.top + handleLength, handlePaint);

        canvas.drawLine(drawRect.left - offset, drawRect.bottom,
                drawRect.left + handleLength, drawRect.bottom, handlePaint);
        canvas.drawLine(drawRect.left, drawRect.bottom + offset,
                drawRect.left, drawRect.bottom - handleLength, handlePaint);

        canvas.drawLine(drawRect.right + offset, drawRect.bottom,
                drawRect.right - handleLength, drawRect.bottom, handlePaint);
        canvas.drawLine(drawRect.right, drawRect.bottom + offset,
                drawRect.right, drawRect.bottom - handleLength, handlePaint);
    }
    ...
    public int getHit(float x, float y) {
        ...
        final float hysteresis = DimensionUtil.dipToPx(viewContext.getContext(), 8);
    ...
    void growBy(float dx, float dy) {
        ...
        // 限制截取框的最小大小
        {
            int minSideLen = DimensionUtil.dipToPx(viewContext.getContext(), 80);
            if (r.width() + 2 * dx < minSideLen) {
                dx = (minSideLen - r.width()) / 2;
                if (maintainAspectRatio) {
                    dy = dx / initialAspectRatio;
                }
            }
            if (r.height() + 2 * dy < minSideLen) {
                dy = (minSideLen - r.height()) / 2;
                if (maintainAspectRatio) {
                    dx = dy * initialAspectRatio;
                }
            }
        ...
        r.inset(-dx, -dy);
        Log.i("HightlightView", r.toString());
